...
from usuarios import cargar_usuarios, puesto_usuario
...
def agregar_autorizacion(entry_consecutivo, combo_tipo, combo_solicitante, entry_puesto, entry_area, entry_fecha_solicitud,
                         ...
                         entry_flimite, tree, listbox_contratos):
    ...
    if not (tipo and solicitante and puesto and area and fecha_solicitud and fecha_requerida and monto and id_proveedor and instruccion and seleccionados):
    ...
    try:
        ...
        (id_autorizacion, tipo_solicitud, solicitante, puesto, area, fecha_solicitud, 
         fecha_requerida, monto, id_proveedor, instruccion, fecha_limite_pago, estado)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'Pendiente')
        """
        valores = (consecutivo, tipo, solicitante, puesto, area, fecha_solicitud, fecha_requerida,
                  monto, id_proveedor, instruccion, limite)
        cursor.execute(query, valores)

        # ✅ Insertar los contratos relacionados
        if not seleccionados:
            messagebox.showwarning("Advertencia", "Selecciona al menos un contrato.")
            return
        contratos_seleccionados = [listbox_contratos.get(i).split(" - ")[0] for i in seleccionados]
        for id_contrato in contratos_seleccionados:
            cursor.execute(
                "INSERT INTO Autorizacion_Contratos (id_autorizacion, id_contrato) VALUES (%s, %s)",
                (consecutivo, id_contrato)
            )
...
def limpiar_formulario(entry_consecutivo, combo_tipo, entry_solicitante, entry_puesto, entry_area, entry_fecha_solicitud, 
                       ...
                       combo_instruccion, entry_flimite, listbox_contratos):
...
def generar_excel(entry_consecutivo, combo_tipo, combo_solicitante, entry_puesto, entry_area, 
                  ...
                  combo_proveedor, combo_instruccion, articulos, tree, entry_flimite, listbox_contratos):
    try:
        ...
        limpiar_formulario(entry_consecutivo, combo_tipo, combo_solicitante, entry_puesto, entry_area, 
                           entry_fecha_solicitud, entry_fecha_requerida, entry_monto, 
                           combo_proveedor, combo_instruccion, entry_flimite, listbox_contratos)
...
def gestionar_autorizaciones():
    ...
    tk.Label(ventana, text="Monto Total (solo valor numérico):", font=("Arial", 10, "bold"), bg="#ffebeb").place(**pos(0.05, 0.60))
    entry_monto = ttk.Entry(ventana)
    entry_monto.place(**pos(0.3, 0.60))
    ...
    tk.Label(ventana, text="Forma de Pago: ", font=("Arial", 10, "bold"), bg="#ffebeb").place(**pos(0.55, 0.60))
    combo_instruccion = ttk.Combobox(ventana, values=["Transferencia Electrónica", "Tarjeta de Débito", "Efectivo"])
    combo_instruccion.place(**pos(0.75, 0.60))

    tk.Label(ventana, text="Limite de Pago: ", font=("Arial", 10, "bold"), bg="#ffebeb").place(**pos(0.55, 0.65))
    entry_flimite = ttk.Entry(ventana)
    entry_flimite.place(**pos(0.75, 0.65))
    ...
    tk.Button(ventana, text="Registrar Autorización", command=lambda: agregar_autorizacion(entry_consecutivo,
        ...
        entry_fecha_requerida, entry_monto, combo_proveedor, combo_instruccion, entry_flimite, tree, listbox_contratos), font=("Arial", 10, "bold")).place(relx=0.48, rely=0.91, relwidth=0.15, relheight=0.05)

    tk.Button(ventana, text="Generar Excel", command=lambda: generar_excel(entry_consecutivo,
        combo_tipo, entry_solicitante, entry_puesto, entry_area, entry_fecha_solicitud, 
        entry_fecha_requerida, entry_monto, combo_proveedor, combo_instruccion, articulos_lista, tree, entry_flimite, listbox_contratos), font=("Arial", 10, "bold")).place(relx=0.65, rely=0.91, relwidth=0.095, relheight=0.05)
